...
int main(int argc, char * argv[]){
    ...
    if(argc<7){
        std::cerr << "Usage: ./two_resource_sim sb NUb ss NUs cv l\n";
        return 1;
    }
    ...
    const int tmax = 10000; // max time to evolve for
    const int teq = 0;
    const int dt = 500; // reporting interval

    const int barcode_dt = 100000000;

    const double Ne = 1e07;
    const double clone_sample_size = 100;
    
    const double s = atof(argv[1]);
    const double Ub = atof(argv[2])/Ne;  
    const double sstrategy = atof(argv[3]);  
    const double Us = atof(argv[4])/Ne;
    
    const int num_resources = 2;
    const double strategy_stddev = atof(argv[5]);
    const double resource_stddev = 1e-03;
    ...
    auto seed_immigration_dfe = SpecialistImmigrationDFE(0, supply_vector.size(), lmax, 0);
    //auto seed_immigration_dfe = GeneralistImmigrationDFE(0, supply_vector.size(), lmax, 0, true);
    
    // Immigration "DFE" for further immigrants over time
    auto immigration_dfe = UniformImmigrationDFE(0, supply_vector.size(), 2, lmax, s);
    
    // DFE that modifies fitnes
    auto fitness_dfe = GaussianDFE(Ub,s,true);
    auto strategy_dfe = create_nested_delta_dfe(TwoResourceBetaDFE(Us, strategy_stddev, lmax,true), sstrategy);
    ...
    for(int replicate_idx=0;replicate_idx<num_replicates;++replicate_idx){
        ...
        for(int t=1; observation_time_ptr < observation_time_end; ++t){
            ...
            for(auto & clone_ptr : population){
                ...
                total_fitness += clone_ptr->size*(clone_ptr->fitness/W0);
                for(auto & expression_level : clone_ptr->expression_levels){
                    harvest_vector[expression_level.resource_idx] += (clone_ptr->size) * (clone_ptr->fitness/W0) * expression_level.level*1.0/clone_ptr->total_expression;
                }
            ...
            if(t%10000==0){
                std::cerr << t << " " << total_size << " " << std::log(W0*total_fitness/total_size) << " " << fixed_mutations.size() << " " << population.size() << std::endl;
            }
            ...
            for(auto & clone_ptr : population){
                ...
                for(auto & expression_level : clone_ptr->expression_levels){
                    ...
                    expected_num_offspring += supply_vector[expression_level.resource_idx]/harvest_vector[expression_level.resource_idx]*(clone_ptr->size) * (clone_ptr->fitness/W0) * expression_level.level*1.0/clone_ptr->total_expression;
            ...
            if(t%100==0){ // slower, so don't do it as often...
