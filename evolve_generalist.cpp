...
int main(int argc, char * argv[]){
    ...
    if(argc<11){
        std::cerr << "Usage: ./program s NUb NUs pscramble Nm p cv k l n0\n";
        return 1;
    }
    ...
    const int tmax = 100000; // max time to evolve for
    const int teq = 0;
    const int dt = 10000; // reporting interval

    const int barcode_dt = 1000;
    ...
    const int initial_num_migrants = atoi(argv[10]);
    
    const bool energy_constraint = false;
    ...
    for(int i=0;i<6;++i){
        times.push_back(long(2*std::pow(10,i)));
        times.push_back(long(5*std::pow(10,i)));
        times.push_back(long(10*std::pow(10,i)));   
    }
    ...
    auto fitness_dfe = GaussianDFE(Ub,s);
